...
| `loading` preso em "Carregando…" | Ligar `loading` no handler e depender do efeito re-disparar trava quando a ação **não muda** filtro (refiltrar o mesmo valor, limpar filtros vazios). Cada ação deve subir um `token` que está nas dependências do efeito e o `loading` deve ser **derivado** (`carregadoToken !== token`) — decisão 7.51. Ainda existe em `useUsers` (dívida registrada em `TASKS.md`). |
