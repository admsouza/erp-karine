...
describe('SubscriptionPaymentService', () => {
  it('publica SubscriptionPaymentReceived somente depois de persistir o pagamento', async () => {
    ...
    const payments = { createInTransaction: vi.fn(async(_data,work)=>{await work(payment,{});return payment;}) };
    ...
    expect(events.publish).toHaveBeenCalledWith(expect.objectContaining({ name: 'SubscriptionPaymentReceived', subscriptionPaymentId: 'payment-1', subscriptionName: 'Plano Ouro' }), expect.anything());
    expect(payments.createInTransaction.mock.invocationCallOrder[0]).toBeLessThan(events.publish.mock.invocationCallOrder[0]);
  });
