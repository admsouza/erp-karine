...
export class FinancialTransactionService {
  ...
  async remove(id:string,user:AuthenticatedUser,requestId?:string,reason?:string){return this.cash.transaction(async tx=>{
    const item=await this.repository.findById(id,tx);if(!item)throw new NotFoundException('Lançamento financeiro não encontrado.');
    if(item.status!=='CANCELADO'){
      if(await this.repository.settlementFor(id,tx))throw new ConflictException('Baixa de conta é imutável; registre ajuste auditado.');
      await this.policy.assertWritable(item.date,tx);const updated=await this.repository.update(id,{status:'CANCELADO',cancelledAt:new Date()},tx);await this.audit.record({actorUserId:user.id,actorName:user.name,actorEmail:user.email,module:'financial',entityType:'FinancialTransaction',entityId:id,action:'CANCELLED',reason,requestId,changes:[{field:'status',before:item.status,after:'CANCELADO'}]},tx);return toFinancialTransactionEntity(updated);
    }
    if(await this.repository.settlementFor(id,tx))throw new ConflictException('Baixa de conta é imutável; não é possível excluir este lançamento.');
    if(await this.repository.hasAdjustments(id,tx))throw new ConflictException('Lançamento com ajustes vinculados não pode ser excluído.');
    await this.policy.assertWritable(item.date,tx);
    await this.repository.remove(id,tx);await this.audit.record({actorUserId:user.id,actorName:user.name,actorEmail:user.email,module:'financial',entityType:'FinancialTransaction',entityId:id,action:'DELETED',reason,requestId,changes:[{field:'status',before:'CANCELADO',after:null}]},tx);return {id};
  });}
