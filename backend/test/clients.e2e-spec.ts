...
describe('Módulo de clientes (e2e)', () => {
  ...
  it('cadastra, pesquisa, edita, inativa e reativa um cliente', async () => {
    ...
    const inactivated = await request(app.getHttpServer())
      .patch(`/api/clients/${created.body.id}/inactivate`)
      .expect(200);
    ...
    const inactives = await request(app.getHttpServer())
      .get('/api/clients')
    ...
    const reactivated = await request(app.getHttpServer())
      .patch(`/api/clients/${created.body.id}/reactivate`)
      .expect(200);
  ...
  it('recusa payload inválido com 400 (CPF, e-mail e campo desconhecido)', async () => {
    const invalidCpf = await request(app.getHttpServer())
      .post('/api/clients')
  ...
  it('responde 404 para cliente inexistente e 400 para id malformado', async () => {
    await request(app.getHttpServer())
      .get('/api/clients/00000000-0000-4000-8000-000000000000')
      .expect(404);

    await request(app.getHttpServer()).get('/api/clients/nao-e-uuid').expect(400);
  });
});
