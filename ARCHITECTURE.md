...
│   ├── procedures/ appointments/ subscriptions/ financial/ protocols/ exams/ dashboard/
│   ├── audit/ users/ integrations/ maintenance/     # seção Sistema (ADMIN)
...
└── shared/                 # só o que é realmente genérico
    ...
    ├── data/               # catálogos sem regra usados por mais de uma feature (decisão 7.50)
...
21 entidades em `backend/prisma/schema.prisma`; o dono de cada uma está em `MODULES.md`.
...
| 7.44 | Caixa, contas a receber/pagar e conciliação vivem **dentro do módulo `financial`**, como submódulos, e os locais do recurso são cadastro (`ResourceAccount`) em vez de campos fixos de espécie/banco/maquineta | São o mesmo domínio e a diferença transacional entre eles não existe: a baixa de uma conta e o fechamento do caixa leem e escrevem os mesmos fatos. Módulos separados teriam que se chamar a cada operação (acoplamento bidirecional proibido) ou duplicar a regra. Já os locais do recurso eram um conjunto fixo no código: a clínica tem mais de uma conta bancária e mais de uma maquineta, então viraram cadastro (`CASH`/`BANK`/`CARD`) com id único — é o que permite transportar saldo **por local** e conciliar **por local**. |
| 7.45 | Saldo inicial do caixa é **transportado** do período fechado; informar saldo inicial à mão é recusado (400) | Se a tela aceitasse digitar o saldo inicial, o fechamento anterior (saldo apurado) e a abertura seguinte (saldo inicial) divergiriam em silêncio, e o caixa deixaria de fechar. O transporte é a regra; o valor conferido entra no fechamento (campo "saldo apurado") e a diferença dele é a divergência. |
| 7.46 | Período fechado **não reabre**: ajuste é lançamento **novo**, vinculado (`adjustmentOfId`) e idempotente (`idempotencyKey`) | Reescrever o período fechado destruiria a conciliação daquele mês (um valor apurado conferido com a gaveta deixa de valer retroativamente). O ajuste preserva os dois fatos: o que foi conferido e o que corrigiu, com motivo e autor na trilha. |
| 7.47 | Evento de domínio passou a ser publicado **dentro da transação** do caso de uso dono (`publish(evento, tx)`) e o barramento o entrega em **série** | "Consistência transacional nas operações financeiras" (pedido do cliente) não se sustenta se o dono confirma o atendimento e o lançamento financeiro pode falhar depois, fora da transação. O `DomainEventBus` aceita `tx` opcional, os handlers financeiros o repassam aos seus repositories, e a entrega sequencial substituiu o `Promise.all` (que não tem ordem nem transação compartilhada). O evento continua opcional para quem não precisa de transação — nenhum contrato público foi quebrado. |
| 7.48 | Conciliação **registra** a divergência e nunca corrige o lançamento automaticamente | A conferência existe para dizer a verdade sobre a gaveta/banco/maquineta, não para "consertar" o sistema. Ajustar automaticamente esconderia a causa e mudaria o saldo conferido. O registro (`FinancialReconciliation`) é append-only, com motivo, e a correção é um ajuste explícito e auditado. |
| 7.54 | `products` guarda **preço de venda e custo de compra simples** (sem série de vigências), uso comercial (`VENDA`/`COMPRA`/`AMBOS`) e o **valor aplicado fica no lançamento**; um lançamento aceita **um item** (procedimento **ou** produto) | Histórico não pode depender do catálogo atual: cada lançamento guarda snapshot de nome e valor. O seletor e a API impõem produto compatível com receita/despesa. Série de vigência só entra se reajustes frequentes se tornarem necessidade. Venda com vários itens continua fora por exigir nova modelagem de relatórios e conciliação. |
| 7.56 | Alteração e exclusão de lançamento financeiro são casos de uso administrativos auditados | `ADMIN` pode editar qualquer lançamento aberto, com motivo obrigatório e todos os campos financeiros principais. `DELETE` primeiro faz cancelamento lógico; quando o lançamento já está cancelado, remove a linha da lista e conserva o evento `DELETED` na trilha. Baixas de conta, lançamentos com ajustes e períodos fechados permanecem protegidos para não destruir conciliação ou títulos. |
| 7.55 | A suíte **e2e roda sequencialmente** (`fileParallelism: false`) por causa do banco de desenvolvimento compartilhado | Em paralelo, uma suíte que limpa o estado de caixa no `beforeAll` apagou dados que a suíte de manutenção estava usando — falha intermitente e de diagnóstico caro. Rodar em série custa ~25s e elimina a classe inteira de flake por concorrência; banco efêmero por suíte seria a solução definitiva, mas é uma mudança maior (registrada para depois). |
| 7.53 | Lançamento manual: **receita → cliente (FK)**, **despesa → credor (texto)**; **`amountCents` é o líquido** e o desconto guarda intenção + efetivo; **pagamento confirmado no salvar** | Três decisões que evitam dado ambíguo: (1) recusar cliente em despesa e credor em receita em vez de aceitar e depois não saber quem é quem — e o credor fica como **texto com sugestão dos já usados**, porque criar cadastro de fornecedores para credor eventual não se paga; (2) o desconto grava `discountType` + `discountValue` (intenção) e `discountCents` (efetivo), mantendo **`amountCents` = líquido**, o que preserva o significado de todo relatório, indicador e conciliação já publicados — `grossAmountCents` fecha a conta; (3) a situação virou **confirmação no salvar** ("já foi recebido/pago?") em vez de um select que o usuário deixava no valor padrão. |
| 7.52 | A **lista de identificações de local** virou cadastro (`ResourceAccountSuggestion`) — mantida em Sistema → Manutenção de cadastros | Ela era fixa no frontend: incluir o banco que a clínica passou a usar dependia de alterar código e publicar. Como é a clínica que conhece seus bancos e maquinetas, a lista virou **dado** com criação/edição/inativação na tela de manutenção (única criação que o hub permite). Continua **sugestão**: o local aceita nome próprio ("Outro (digitar)"), então o catálogo não é uma lista fechada. Unicidade **sem diferenciar maiúsculas nem acentos** (`entities/resource-name.ts`): o índice único do Postgres é sensível a acento e `Itau`/`Itaú` virariam dois bancos na conferência de caixa. A migração semeia as nove identificações que estavam no código (`ON CONFLICT DO NOTHING`, idempotente). |
| 7.50 | Catálogos de **dados de referência** (ex.: identificações de local do recurso) ficam em `shared/data/`, não dentro de uma feature | A lista de locais (espécie, bancos, maquinetas) é usada por **duas features** — o cadastro em `financial` e a correção em `maintenance`. Deixá-la dentro de `financial` obrigaria a `maintenance` a importar detalhe interno de outra feature (proibido) ou a **duplicar a lista** — e a experiência já mostrou o custo: uma lista envelhece e a outra não (o cliente pediu para trocar os bancos duas vezes). `shared/` recebe aqui **dado sem regra**: quem valida nome/tipo é o backend. |
| 7.51 | `loading` de lista **derivado de um token**, nunca ligado à mão dentro do handler | O padrão antigo (`setLoading(true)` no handler + efeito que busca quando o filtro muda) trava em "Carregando…" quando a ação **não muda** nenhum filtro — refiltrar o mesmo valor, limpar filtros já vazios, voltar para a mesma página. O efeito não re-dispara e ninguém desliga o `loading`. Agora cada ação sobe um `token` (que está nas dependências do efeito) e `loading` é `carregadoToken !== token`: nunca fica preso, mesmo sem mudança de valor. |
| 7.49 | **Manutenção de cadastros** (`maintenance`) é um hub **sem tabela e sem regra própria** na seção Sistema, que consulta e delega aos módulos donos | O pedido é operacional: corrigir e inativar/reativar cadastros básicos sem caçar a tela de cada módulo. Criar um módulo com tabela própria violaria a regra de dependência (dois módulos escrevendo no mesmo dado) e um interceptor genérico de `PATCH` perderia o significado de negócio. Então: `GET` monta a lista pelos **contratos públicos de consulta**, `PATCH` repassa ao **serviço público do dono** (que continua validando) e o hub **só registra trilha quando o dono não registra** — hoje `financial` já registra o local do recurso, e duplicar confundiria a leitura da auditoria. Campos editáveis por tipo são deliberadamente poucos: identificação e o dado principal (valor unitário de procedimento fica de fora porque tem vigência própria). |
