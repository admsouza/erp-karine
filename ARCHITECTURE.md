...
| 7.56 | Alteração e exclusão de lançamento financeiro são casos de uso administrativos auditados | `ADMIN` pode editar qualquer lançamento aberto, com motivo obrigatório e todos os campos financeiros principais; `DELETE` faz exclusão lógica (`CANCELADO`), nunca remove a linha. Período fechado e baixa de conta seguem imutáveis: o sistema recusa a reescrita para não destruir conciliação ou títulos. |
